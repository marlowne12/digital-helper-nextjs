...
You just installed your AI platform. Your agent works, but it's generic — it doesn't know who it is, who you are, or what it did yesterday. Three files fix that.
...
### Session Size and Longevity

Long-running main sessions accumulate context and will eventually hit API rate limits or degrade in quality. This isn't hypothetical — it's inevitable.

- **Watch for warning signs:** Session running for multiple days, heavy tool use, large file reads, lots of cron/subagent activity piling up context.
- **Rotate proactively.** When a session gets heavy, suggest archiving and starting fresh *before* it breaks. Don't wait for the rate limit error.
- **Recovery is trivial.** Archive the old session, start a new one. Memory files persist across sessions — that's the whole point of writing things down.
- **For high-traffic channels,** consider a weekly "fresh session" as standard practice. Scheduled rotation beats emergency recovery every time.
- **Prevention beats recovery.** A clean restart takes 30 seconds. Debugging a degraded session takes an hour and produces worse results the whole time.
...
### Beyond Sub-Agents: Agent Orchestration Frameworks

Sub-agents handle one-off tasks. But once you have 3+ agents running regularly — each with persistent responsibilities, workflows, and accountability — you've outgrown the sub-agent pattern.

Agent orchestration frameworks (like Paperclip) add the layer above: org charts, budgets, audit logs, and structured delegation across agent teams. The mental model: if your AI platform is an employee, an orchestration framework is the company. Worth evaluating once your agent fleet grows beyond ad-hoc spawning.
...
### MEMORY.md as a Routing Document

Keep MEMORY.md under 200 lines. It should *route* to topic files, not store everything directly.

```
MEMORY.md (compact — loads every session)
├── Who the user is (5 lines)
├── Active projects (one line each, with pointers)
├── Key preferences and rules (10 lines)
└── Pointers to detail files:

memory/ventures.md ← full venture details
memory/lessons-learned.md ← tactical rules and gotchas
memory/infrastructure.md ← system details, API notes
```

A bloated MEMORY.md burns tokens on every session load. A tight routing doc loads fast and points you exactly where you need to go. When MEMORY.md hits 200 lines, extract topic clusters into their own files and replace with one-line pointers.

### The Curation Problem

Daily notes accumulate. After a month, 30 files. After a year, 365. You can't read them all. This is where curation comes in — periodically reviewing daily notes and distilling lasting knowledge into MEMORY.md (and its topic files).

But doing this well is its own discipline: when to run maintenance, how to handle multi-project isolation, what to do when concurrent sessions write to the same file, how to keep MEMORY.md lean as context grows, and how to secure personal information across session types.
